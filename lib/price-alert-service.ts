...
export async function getUserAlerts(userId: string): Promise<PriceAlert[]> {
  ...
  for (const alertId of alertIds) {
    ...
    if (alertData) {
      // If alertData is already an object, use it directly, otherwise parse as JSON
...
async function getCurrentPrice(tokenAddress: string): Promise<number | null> {
  try {
    ...
    const response = await fetch(`https://api.monorail.xyz/v2/token/${tokenAddress}/price`, {
      headers: {
